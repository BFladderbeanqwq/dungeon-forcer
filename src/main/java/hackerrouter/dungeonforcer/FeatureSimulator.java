...
/**
 * Simulates MonsterRoomFeature.place() execution.
 * Handles shell placement, chest placement, spawner type determination,
 * and tracks block changes for backtracking.
 *
 * Key: the RNG passed in is the SHARED random used by the entire placement chain.
 * After place() returns, the RNG state reflects all consumed calls.
 */
public class FeatureSimulator {
    ...
    /**
     * Simulate MonsterRoomFeature.place().
     * Consumes RNG for sizeX, sizeZ, and (if check passes) shell/chest/spawner.
     *
     * @param random shared RNG — state is modified
     * @param originX world X of dungeon origin
     * @param originY world Y of dungeon origin
     * @param originZ world Z of dungeon origin
     * @param isDeep whether this is a deep dungeon attempt
     * @param attemptIndex index within the feature's attempts
     * @return Spawner if dungeon passes checks and is placed, null if check fails
     */
    public PlaceResult simulate(WorldgenRandom random, int originX, int originY, int originZ,
        ...
        // sizeX and sizeZ consumed from shared RNG
        ...
        // === Check conditions (clamped to chunk) ===
        DungeonCheckResult check = checkConditions(lx, lz, originY, sizeX, sizeZ);
        if (!check.pass) {
            // Check failed — no more RNG consumed, return null
            return new PlaceResult(null, check, sizeX, sizeZ, lx, lz);
        }

        // === Shell placement (consumes RNG for mossy cobblestone) ===
        // Save RNG state after shell for floorBlocksNeeded iteration
        simulateShell(random, lx, lz, originY, sizeX1, sizeZ1);
        long shellRngLo = random.getSeedLo();
        long shellRngHi = random.getSeedHi();

        // === Chest placement (consumes RNG) ===
        simulateChests(random, lx, lz, originY, sizeX, sizeZ);

        // === Spawner type ===
        // Util.getRandom(MOBS, random) = MOBS[nextInt(4)]
        int mobIndex = random.nextInt(4);
        SpawnerType type = SpawnerType.fromMobIndex(mobIndex);

        // spawner.setEntityId(type, random) → getOrCreateNextSpawnData → spawnPotentials.getRandom(random)
        // For a new spawner with 1 spawn potential entry, this consumes nextInt(1)
        random.nextInt(1);

        // === Place spawner in buffer ===
        int sbx = lx + 4;
        int sby = worldToBufferY(originY);
        int sbz = lz + 4;
        setAndRecordChest(sbx, sby, sbz, DungeonFinder.BLOCK_SPAWNER);

        // Build spawner object
        ...
        // Add render data: outside-chunk floor/ceiling blocks
    ...
    /**
     * Undo all changes (shell + chests) made by the last simulate() call.
     */
    public void undoAll() {
        undoChests();
    ...
    /**
     * Undo only chest changes (for floorBlocksNeeded iteration).
     */
    ...
    private DungeonCheckResult checkConditions(int lx, int lz, int oy, int sizeX, int sizeZ) {
        ...
        // Check floor/ceiling solid within chunk
    ...
    private void simulateShell(WorldgenRandom rng, int lx, int lz, int oy, int sizeX1, int sizeZ1) {
        ...
        // Only process inside-chunk positions for RNG consumption.
        // Outside-chunk RNG differences are covered by floorBlocksNeeded iteration.
    ...
    private void simulateChests(WorldgenRandom rng, int lx, int lz, int oy, int sizeX, int sizeZ) {
        for (int cc = 0; cc < 2; cc++) {
            for (int i = 0; i < 3; i++) {
                ...
                int cbz = cz + 4;
                // vanilla checks ALL positions (including outside chunk)
                if (getBlock(cbx, cby, cbz) == DungeonFinder.AIR) {
                    ...
                    if (wallCount == 1) {
                        ...
                        rng.nextLong(); // loot table RNG (RandomizableContainer.setBlockEntityLootTable)
                        break;
                    }
                }
            }
    ...
    public static class PlaceResult {
        public final Spawner spawner; // null if check failed
